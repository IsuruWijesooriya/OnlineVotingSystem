...
Class Action {
	...
	function del_user(){
		...
		$data .= ", password = '$password' ";
		$data .= ", type = '$type' ";
		$del = $this->db->query("DELETE FROM users where id = ".$id);
		if($del){
			return 1;
		}
	}
	...
	function delete_voting(){
		...
		$path = $this->db->query("SELECT * FROM voting_opt where voting_id=".$id)->fetch_array()['image_path'];
		$delete = $this->db->query("DELETE FROM voting_opt where voting_id=".$id);
		$delete = $this->db->query("DELETE FROM votes where voting_id=".$id);
		if($delete){
